...
mkdir -p ../data/raw/energy_burden/counties/AMI
mkdir -p ../data/raw/energy_burden/counties/FPL
...
# Extract AMI and FPL county-level files into separate folders
for f in ../data/raw/energy_burden/zips/*.zip; do
  unzip -j "$f" "*AMI*Counties*" -d ../data/raw/energy_burden/counties/AMI
  unzip -j "$f" "*FPL*Counties*" -d ../data/raw/energy_burden/counties/FPL
done